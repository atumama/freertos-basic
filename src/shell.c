...
void new_command(int, char **); // declare a 'new' command 
int fib(int);
...
cmdlist cl[]={
	...
	MKCL(new, "create a new FreeRTOS task"),  
	MKCL(, ""),
};
...
void test_command(int n, char *argv[]) {
	...
	if(n==2){ //n is for the number of parameters you key in 
		if(!strcmp(argv[1], "showfib")){
			fio_printf(1, "The first twenty terms of fibonacci sequence are:\r\n");
			for(int i = 0;i < 20;i++){
				fio_printf(1, "%d\r\n", fib(i));
			}
		}
...
void new_command(int n, char *argv[]) {
	xTaskCreate(test_task, (signed portCHAR *) "This is test", 512, NULL, tskIDLE_PRIORITY + 1, NULL);
	fio_printf(1, "\r\n");
}
...
int fib(int x){ //recursive fibonacci
	if(x<=0) 
		return 0;
	if(x==1) 
		return 1;
